...
class gogoChipC:
    ...
    def compile(self, LogoCodeString, NetworkDeviceList):
        ...
        # Run the Logo parser
        # ======================================
        self.logo_parser = tinkerParser(self.fpp.procedures_dict, self.fpp.ordered_procedures_list, NetworkDeviceList)
...
#             'SWITCH2':			58,
             ...
             'STOPALL':		    68,
             'GETREGISTER':    69,         # returns a device register value
             'SETREGISTER':    70,         # sets a device register value
             'LOWBYTE':		    71,
             'HIGHBYTE':		72,
             'OP_BITWISE_AND':   73,
             'OP_BITWISE_OR':    74,
             'OP_BITWISE_NOT':   75,
             'OP_BITWISE_XOR':   76,
             'OP_BITWISE_SHIFT_LEFT':   77,
             'OP_BITWISE_SHIFT_RIGHT':  78,
             # 'SENSOR3':			73,
             # 'SENSOR4':			74,
             # 'SENSOR5':			75,
             # 'SENSOR6':			76,
...
#        'isthatway'     : 'IS_THATWAY',
    ...
    #    'LIST_BLOCK',
        ...
        'OP_BITWISE_AND', 'OP_BITWISE_OR','OP_BITWISE_NOT','OP_BITWISE_XOR','OP_BITWISE_SHIFT_LEFT','OP_BITWISE_SHIFT_RIGHT',
        'NEWIR', 'NEWSERIAL',
        'NEW_REQUEST',
        'IS_ON','IS_OFF','IS_THISWAY','IS_THATWAY',
        'PORT_ON','PORT_OFF',
    ...
    def t_OP_BITWISE_AND(self, t):
        r'&'
        t.value = "OP_BITWISE_AND"
        return t
    def t_OP_BITWISE_OR(self, t):
        r'\|'
        t.value = "OP_BITWISE_OR"
        return t
    def t_OP_BITWISE_NOT(self, t):
        r'~'
        t.value = "OP_BITWISE_NOT"
        return t
    def t_OP_BITWISE_XOR(self, t):
        r'\^'
        t.value = "OP_BITWISE_XOR"
        return t
    def t_OP_BITWISE_SHIFT_LEFT(self, t):
        r'<<'
        t.value = "OP_BITWISE_SHIFT_LEFT"
        return t
    def t_OP_BITWISE_SHIFT_RIGHT(self, t):
        r'>>'
        t.value = "OP_BITWISE_SHIFT_RIGHT"
        return t
...
class tinkerParser:
    ...
    # lower rules have high precedence
    # this follows the standard C operator precedence
    precedence = (
        ...
        ('right', 'NAME'),
        ('right', 'RANDOM', 'LOWBYTE', 'HIGHBYTE', 'I2C_READ', 'READ_SENSOR', 'READ_SWITCH', 'ASKPORT', 'GET_REGISTER', 'PORT_ON', 'PORT_OFF'),
        ('left', 'OP_OR', 'OP_XOR'),
        ('left', 'OP_AND'),
        ('left', 'OP_BITWISE_OR'),
        ('left', 'OP_BITWISE_XOR'),
        ('left', 'OP_BITWISE_AND'),
        ('left', 'OP_EQUAL'),
        ('left', 'OP_LESS', 'OP_LESS_OR_EQUAL', 'OP_GREATER', 'OP_GREATER_OR_EQUAL'),
        ('left', 'OP_BITWISE_SHIFT_LEFT', 'OP_BITWISE_SHIFT_RIGHT'),
        ('left', 'OP_PLUS', 'OP_MINUS'),
        ('left', 'OP_MULTIPLY', 'OP_DIVISION', 'OP_MODULO'),
        ('right', 'OP_NOT'),
        ('right', 'OP_BITWISE_NOT'),
    ...
    def __init__(self, procedurs_dict, ordered_procedures_list, network_device_list):
        ...
        # ======================================
        # Load the request dictionary from file
        # ======================================
        self.reqDict = self.loadRequestDict()


    def saveRequestDict(self, dictToSave):
        with open('dict.pkl', 'wb') as f:
            pickle.dump(dictToSave, f, 0)

    def loadRequestDict(self):
    ...
    def p_statement_two_number_parameters(self,t):
        ''' statement   :   SET_REGISTER    expression  expression
        '''
        t[0] = t[3] + ', ' + t[2] + ', ' + t[1]
    ...
    def p_statement_askport(self, t):
        ' statement : ASKPORT '
    ...
    def p_expression_port_on(self, t):
        ''' expression      :    PORT_ON expression
        '''
        t[0] = t[2] + ", ISON"

    def p_expression_port_off(self, t):
        ''' expression      :    PORT_OFF expression
        '''
        t[0] = t[2] + ", ISOFF"
    ...
    def p_expression_binop(self, t):
        '''expression : expression OP_PLUS expression
                      ...
                      | expression OP_BITWISE_SHIFT_LEFT expression
                      | expression OP_BITWISE_SHIFT_RIGHT expression
        '''
...
#            raise SyntaxError
    ...
    def p_expression_decentralized_operation(self,t):
        '''  expression : NEW_REQUEST
                        | REQUEST_DATA
        '''
        t[0] = t[1]

    def reqString2reqID(self, reqString):
        if reqString in self.reqDict:
            request_id = self.reqDict[reqString]
        else:
            request_id = len(self.reqDict)
            self.reqDict[reqString] = request_id
            self.saveRequestDict(self.reqDict)
        return request_id

    def p_expression_decentralized_request_id(self, t):
        '''expression :  REQUEST OP_EQUAL STRING
        '''

        # convert the request string to a request id using a dictionary
        request_id = self.reqString2reqID(t[3])

        t[0] = t[1] + ', NUM8, ' + str(request_id) + ', OP_EQUAL'

    def p_statement_ask(self, t):
        ''' statement   : ASK STRING list_open STRING expression list_close
        '''

        try:
            targetID = self.network_device_list.index(t[2])
        except ValueError:
        ...
        # convert the request string to a request id using a dictionary
        request_id = self.reqString2reqID(t[4])
        requestData = t[5]

        t[0] = requestData + ', NUM8, ' + str(request_id) + ', NUM8, ' + str(targetID) + ', ' + t[1]
    ...
    def p_expression_one_parameter(self, t):
        ''' expression      :    LOWBYTE expression
                             ...
                             |   GET_REGISTER expression

        '''
        t[0] = t[2] + ", " + t[1]

    def create_talkto_bytecode(self, inString):
        ' converts abcd: -> NUM8 0b1111 ASKPORT'
        ...
        return "NUM8, " + str(talk_to_value) + ", ASKPORT"
...
# ===========================================================================
# The following example demonstrates how to use the Logo Compiler.
# - input: Logo code is contained in the text file "logo.txt"
# - output: print out of the compiled binary code in many levels.
#   symbolic and actual binaries.
#
...
if __name__ == '__main__':
    ...
    # ======================================
    logoString = open('logo.txt', 'r').read()
